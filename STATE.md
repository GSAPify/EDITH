...
**Current phase:** Slices 0–6 DONE (Memory+Brain, PR-review, Voice, Session-awareness, Router, **Desktop-control**) + Viewer + Ingest + NL-finder + **Workspace-graph (multi-org)**. **ALL NUMBERED SLICES BUILT.** Next: **voice conversation mode** (`docs/specs/voice-conversation-mode.md`) — the daemon-integration "she talks back" gap: route voice through Brain for cross-turn memory + follow-up window + silence endpointing + mute toggle.
**Active slice:** → all numbered slices done; next is **voice conversation mode** (spec ready). This session (2026-07-12, Session 16): built **Slice 6 (Desktop control)** advisor-first + TDD on branch `feat/slice-6-desktop-control` (PR-ready) — `edith/desktop/` (regex `parse_command` + filesystem `RepoResolver` + osascript/`open` executors behind an injected `Runner` seam) + `DesktopControlSkill` registered LAST in edithd's Brain. 258 tests + 2 skipped, ruff/pyright clean. Terminal.app `do script` drives BOTH the visible-terminal and the OMC-launch paths (claude is an interactive TTY REPL, won't run headless under `Popen` — spec Option-C deviation). `RepoResolver` prefers the flat `~/gitstuff/<name>` over the org-nested duplicate (bulk-clone artifact, verified identical remote); same-depth collisions still ASK. Safe live checks: parsed all spec examples, `osacompile`d the AppleScript clean, resolver ran against the real 1400-repo tree. **Owner LIVE-SMOKE of actual OS actions (Spotify/Terminal/OMC) still pending.** Prior this session: multi-org workspace graph (PR #10), voice self-echo/persona fixes (PRs #8/#12), SageMaker `hey_edith` retrain, Router Slice 5 (PR #7).
