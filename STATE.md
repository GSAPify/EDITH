...
**Current phase:** Slices 0–6 DONE (Memory+Brain, PR-review, Voice, Session-awareness, Router, **Desktop-control**) + Viewer + Ingest + NL-finder + **Workspace-graph (multi-org)**. **ALL NUMBERED SLICES BUILT.** Voice conversation mode also done (Session 17). Next: route voice → Brain + real Kuzu in the `edithd` composition root (semantic recall + cross-session remember — the "she talks back" daemon gap).
